...
# Configuration
TOKEN_SERVICE_URL = "https://your-strava-mcp-service.vercel.app"  # Update with your deployed service URL
CLIENT_ID = "26565"  # Your Strava app's client ID (public, can be hardcoded)
REPO_URL = "https://github.com/your-username/strava-mcp.git"  # Update with your repo
...
def update_claude_config(mcp_path: Path, access_token: str, refresh_token: str) -> bool:
    ...
    if config_path.exists():
        try:
            ...
            log('Warning: Could not parse existing Claude config, creating new one', Colors.YELLOW)
...
def authenticate_with_strava() -> tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log(f'✅ Authentication successful! Welcome, {athlete.get("firstname", "athlete")}!', Colors.GREEN)
...
"""
    ...
    # Run sync script
    python_exe = install_path / "venv" / ("Scripts/python.exe" if platform.system() == "Windows" else "bin/python")
    
    # Use --pages 5 to limit initial sync (500 activities max)
    sync_success = run_command(f'"{python_exe}" strava-sync.py --pages 5', str(install_path))
...
def restart_claude_desktop() -> bool:
    ...
    try:
        ...
        if system == "Darwin":  # macOS
            ...
            time.sleep(2)
            subprocess.run(['open', '/Applications/Claude.app'], check=False, capture_output=True)
            
        elif system == "Windows":
            subprocess.run(['taskkill', '/F', '/IM', 'Claude.exe'], check=False, capture_output=True)
            time.sleep(2)
...
def main():
    ...
    if not run_command(f'git clone {REPO_URL} .', str(install_path)):
        log('❌ Failed to clone repository. Please check the repo URL and try again.', Colors.RED)
        sys.exit(1)
    ...
    restart_choice = ask_question('Would you like to restart Claude Desktop now? (y/n): ')
    
    if restart_choice.lower().startswith('y'):
        restart_success = restart_claude_desktop()
        if restart_success:
            log('\n✅ Claude Desktop has been restarted!', Colors.GREEN)
            log('The Strava MCP should now be available in your conversations.', Colors.GREEN)
        else:
            log('\n⚠️  Automatic restart failed. Please restart Claude Desktop manually.', Colors.YELLOW)
    else:
    ...
    log(f'\n🔗 Token service: {TOKEN_SERVICE_URL}')
