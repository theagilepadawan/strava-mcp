...
TOKEN_SERVICE_URL = os.getenv(
    "STRAVA_TOKEN_SERVICE_URL", "https://your-strava-mcp-service.vercel.app"
)
DB_PATH = os.getenv(
    "STRAVA_DB_PATH",
...
def ensure_token() -> TokenInfo:
    ...
    if not token:
        ...
        if not REFRESH_TOKEN:
            raise ValueError(
                "No refresh token available. Please re-run the setup tool."
            )
...
def format_activity_for_display(activity_data):
    ...
    formatted_data = {
        ...
        "average_speed_kph": (activity_data.get("average_speed", 0) * 3.6)
        if "average_speed" in activity_data
        else None,
        "max_speed_kph": (activity_data.get("max_speed", 0) * 3.6)
...
def query_strava_database(
    sql: str, params: Optional[List[Any]] = None, limit: int = 300, offset: int = 0
) -> str:
    """
...
if __name__ == "__main__":
    ...
    if (
        not TOKEN_SERVICE_URL
        or TOKEN_SERVICE_URL == "https://your-strava-mcp-service.vercel.app"
    ):
        logger.error(
            "STRAVA_TOKEN_SERVICE_URL environment variable must be set to your deployed service!"
        )
        exit(1)
